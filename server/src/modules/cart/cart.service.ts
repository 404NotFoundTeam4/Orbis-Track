...
import {
  AccessoriesSchema,
  ApprovalFlowsSchema,
  ApprovalFlowStepsSchema,
  BorrowReturnTicketsSchema,
  UpdateCartDeviceDetailDataSchema,
  CartDeviceChildSchema,
  CartItemSchema,
  CartSchema,
  CategoriesSchema,
  CreateBorrowTicketPayload,
  CreateBorrowTicketStagePayload,
  CreateTicketDevicePayload,
  DeviceChildSchema,
  DeviceSchema,
  IdParamDto,
  TicketDevicesSchema,
...
async function getCartItem(params: IdParamDto) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const cdc of cart_device_childs) {
      ...
      if (!cartDeviceChildsByCtiId.has(key))
        cartDeviceChildsByCtiId.set(key, []);
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const newBorrowTicket = await tx.borrow_return_tickets.create({
      data: {
        ...
        brt_end_date: cartItem.cti_end_date ?? new Date(),
    ...
    await tx.log_borrow_returns.create({
      data: {
        ...
        lbr_note: `${user?.us_firstname ?? "Unknown"} ${user?.us_lastname ?? "User"} ส่งคำร้องการยืมอุปกรณ์`,
        ...
        const [ticketDevice, availability] = await Promise.all([
          ...
          tx.device_availabilities.create({
            data: {
              ...
              da_start: cart_item.cti_start_date ?? new Date(),
              da_end: cart_item.cti_end_date ?? new Date(),
              da_status: "ACTIVE",
              created_at: new Date(),
            },
            select: {
 ...
 * Author : Nontapat Sinhum (Guitar) 66160104
 **/
async function createBorrowTicketStages(
  params: CreateBorrowTicketStagePayload,
) {
  const { cartItemId, borrowTicketId } = params;
