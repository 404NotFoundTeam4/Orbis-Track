...
const ListDevices = () => {
  ...
  const filtered = useMemo(() => {
    ...
    const result = devices.filter((device) => {
      ...
      const byCategory =
        !categoryFilter?.value || device.category === categoryFilter.value;
      const byDepartment =
        !departmentFilter?.value ||
        device.department === departmentFilter.value;
      const bySection =
  ...
  useEffect(() => {
    setPage(1);
  }, [searchFilter, categoryFilter, departmentFilter, sectionFilter]); // เปลี่ยนกรอง/เรียง → กลับหน้า 1
  ...
  return (
    <div className="flex-1 flex flex-col gap-[20px] w-full h-[calc(100vh-140px)] overflow-hidden p-4 ">
      ...
      {/* เพิ่มให้สามารถ scroll ได้ */}
      <div className="flex-1 overflow-y-auto pr-2">
        {/* Card */}
        <div className="min-h-[680px]">
          <div className="grid grid-cols-5 gap-x-[30px] gap-y-[24px]">
            {pageRows.map((device) => (
              <DevicesCard key={device.de_id} device={device} />
            ))}
          </div>
        </div>
