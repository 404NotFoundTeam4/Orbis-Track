...
import DeviceManageModalRepair, { type RepairDeviceUpdate } from "./DeviceManageModalRepair";
...
export default function RequestItemRepair({
  ...
  const [localStatus, setLocalStatus] = useState<RepairTicketStatus | string>(ticket.status);
   ...
   * Author : Worrawat Namwat (Wave) 66160372
   */
  const mappedDevicesForManage = ticket.device_info.reported_devices?.map((d) => ({
    id: d.id, 
    ...
    <div className="w-full bg-white overflow-hidden transition-all duration-300">
      ...
      >
        <div className="flex flex-col pl-2 overflow-hidden pr-2 ">
          <span className="text-[#000000] font-medium truncate ml-1" title={ticket.device_info.name}>
            {ticket.device_info.name}
          </span>
          <span className="text-[#9E9E9E] text-xs mt-0.5 ml-1">
        ...
        <div className="text-[#000000] truncate " title={ticket.device_info.category || "-"}>
        ...
        <div >
          <span className={`flex items-center justify-center -ml-3 w-[105px] h-[44px] border rounded-full text-sm font-medium ${currentStatus.className}`}>
            {currentStatus.label}
          </span>
        </div>
        ...
        <div className="flex" onClick={(e) => e.stopPropagation()}>
          {activeTabKey === "mine" ? (
            ...
            <div className="flex flex-col text-center">
              ...
              <span 
                className="text-[#73D13D] flex items-start justify-start px-[2px] py-1 text-[16px] truncate max-w-[120px] -ml-3" 
                title={localApprover}
              >
                {localApprover}
              </span>
            ...
            <div className="mx-6 pb-8 pt-4 grid grid-cols-[300px_1fr] gap-x-10 gap-y-6 items-start">
              <div className="w-[300px] flex flex-col gap-2 shrink-0">
                ...
                <button 
                  ...
                  className={`w-full h-[36px] border border-[#D9D9D9] rounded-lg text-sm transition-colors ${
                    deviceImage 
