 ...
 */
import { z } from "zod";
import * as dotenv from "dotenv"; // โหลดค่าจากไฟล์ .env เข้าสู่ process.env อัตโนมัติ

process.env.NODE_ENV = process.env.NODE_ENV ?? "development";

if (process.env.NODE_ENV === "development") {
  // ถ้าเป็น 'development', ให้โหลดไฟล์ .env.dev
  dotenv.config({ path: ".env.dev" });
} else {
  // ถ้าเป็น 'production' หรือ 'test', ให้โหลด .env (หรือปล่อยให้ system vars ทำงาน)
  dotenv.config(); // โหลด .env ปกติ
}
